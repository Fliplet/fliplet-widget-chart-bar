...
Fliplet.Widget.instance('chart-bar-1-1-0', function(data) {
  ...
  function init() {
          ...
          switch (data.dataSourceQuery.selectedModeIdx) {
            ...
            default:
              ...
              result.dataSourceEntries.forEach(function(row, i) {
                ...
                data.columns.push(row[data.dataSourceQuery.columns.category] || T('widgets.charts.bar.category', { count: i + 1 }));
            ...
            case 1:
              // Summarize data
              data.name = T('widgets.charts.bar.title', { name: data.dataSourceQuery.columns.column });
          ...
          tooltip: {
            ...
            pointFormatter: function() {
              return [
                ...
                T('widgets.charts.bar.seriesName', { name: this.series.name }),
                '</span>',
                TN(this.y)
              ].join('');
            },
    ...
    refreshData().then().then(drawChart).catch(function(error) {
      console.error(error);
      setRefreshTimer();
    });
